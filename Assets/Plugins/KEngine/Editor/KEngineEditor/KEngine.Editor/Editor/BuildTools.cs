...
//                        AppEngine.GetConfig(KEngineDefaultConfigs.AssetBundleExt
        ...
        public static void BuildAllAssetBundles()
        {
            ...
            CopySceneToBundleResource(); //Loy: 把Assets/Art/Scenes目录下的所有场景拷贝至BundleResources目录下，以备打包
		...
		public static void BuildAllWindowsAssetBundles()
		{
        ...
        const string RES_MAPPING_FILE = "PackageNameMapping.xml";
        static string BaseABPath = Path.GetFullPath(Application.streamingAssetsPath + "\\Bundles\\" + KEngine.KResourceModule.BuildPlatformName + "\\");
        ...
        public static void GenerateMappingFile()
        {
            ...
            string mappingFilepath = Path.Combine(Application.dataPath + "/../../Product", RES_MAPPING_FILE);
            if (File.Exists(mappingFilepath))
            {
                File.Delete(mappingFilepath);
            }

            StreamWriter mappingFile = new StreamWriter(mappingFilepath);
            mappingFile.WriteLine("<root>");



            Dictionary<string, string> dict = new Dictionary<string, string>();
            foreach (string path in Directory.GetFiles(Application.dataPath, "*.manifest", SearchOption.AllDirectories))
            {
                string basePath = Path.GetFullPath(path).Replace(BaseABPath, "").Replace(".manifest", "");
                string[] lines = File.ReadAllLines(path);
                foreach (string line in lines)
                {
                    if (line.StartsWith("- Assets/"))
                    {
                        string resName = Path.GetFileName(line.Substring(2)).Replace("&", "_");
                        mappingFile.WriteLine("<item k=\"" + resName.ToLower() + "\" v=\"" + basePath + "\" />");
                    }
                }
            }
            mappingFile.WriteLine("</root>");
        ...
        [MenuItem("KEngine/AssetBundle/Build Local")]
        public static void BuildLocalAllAssetBundles()
        {
            if (EditorApplication.isPlaying)
            {
                Log.Error("Cannot build in playing mode! Please stop!");
            ...
            string pathUrl = KResourceModule.GetAppDataPath();
            string[] pathUrls = pathUrl.Split('/');
            string outputPath = "";
            for (int i = 0; i < pathUrls.Length-2; i++)
            {
                string url = pathUrls[i];
                outputPath += url + "/";
            }
            outputPath += "cynet/少年群侠传3D-loy/Bundles/Windows/";

//            Debug.Log("=========path============= "+ pathUrl+"___"+ outputPath);

//            var outputPath = "C:/Users/Administrator/AppData/LocalLow/cynet/少年群侠传3D-loy/Bundles/Windows/";//GetExportPath(EditorUserBuildSettings.activeBuildTarget);
            Log.Info("Asset bundle start build to: {0}", outputPath);
            BuildPipeline.BuildAssetBundles(outputPath, BuildAssetBundleOptions.DeterministicAssetBundle, EditorUserBuildSettings.activeBuildTarget);
            Log.Info("Asset bundle build success.");
        }

        [MenuItem("KEngine/AssetBundle/Build Local(UI)")]
        public static void BuildLocalUIAssetBundles()
        {
            if (EditorApplication.isPlaying)
            {
                Log.Error("Cannot build in playing mode! Please stop!");
...
//            CopySceneToBundleResource(); //Loy: 把Assets/Art/Scenes目录下的所有场景拷贝至BundleResources目录下，以备打包
        ...
        [MenuItem("KEngine/AssetBundle/Build Local(Scene)")]
        public static void BuildLocalSceneAssetBundles()
        {
            if (EditorApplication.isPlaying)
            {
                Log.Error("Cannot build in playing mode! Please stop!");
        ...
        [MenuItem("KEngine/AssetBundle/Build Local(Effect)")]
        public static void BuildLocalEffectAssetBundles()
        {
            if (EditorApplication.isPlaying)
            {
                Log.Error("Cannot build in playing mode! Please stop!");
        ...
        [MenuItem("KEngine/AssetBundle/Build Local(Character)")]
        public static void BuildLocalCharacterAssetBundles()
        {
            if (EditorApplication.isPlaying)
            {
                Log.Error("Cannot build in playing mode! Please stop!");
        ...
        public static void MakeUIAssetBundleNames()
        {
            var dir = ResourcesBuildDir;
            // set BundleResources's all bundle name
            foreach (var filepath in Directory.GetFiles(dir, "*.*", SearchOption.AllDirectories))
            {
                ...
                if (filepath.Contains("BundleResources/UI"))
                {
//                    Debug.Log("============================== "+ filepath);
                    var bundleName = filepath.Substring(dir.Length, filepath.Length - dir.Length);
                    importer.assetBundleName = bundleName + AppEngine.GetConfig(KEngineDefaultConfigs.AssetBundleExt);
        ...
        public static void MakeSceneAssetBundleNames()
        {
            var dir = ResourcesBuildDir;
            // set BundleResources's all bundle name
            foreach (var filepath in Directory.GetFiles(dir, "*.*", SearchOption.AllDirectories))
            {
                ...
                if (filepath.Contains("BundleResources/Scenes"))
                {
                    var bundleName = filepath.Substring(dir.Length, filepath.Length - dir.Length);
                    importer.assetBundleName = bundleName + AppEngine.GetConfig(KEngineDefaultConfigs.AssetBundleExt);
                }
        ...
        public static void MakeEffectAssetBundleNames()
        {
            var dir = ResourcesBuildDir;
            // set BundleResources's all bundle name
            foreach (var filepath in Directory.GetFiles(dir, "*.*", SearchOption.AllDirectories))
            {
                ...
                if (filepath.Contains("BundleResources/Effect"))
                {
                    var bundleName = filepath.Substring(dir.Length, filepath.Length - dir.Length);
                    importer.assetBundleName = bundleName + AppEngine.GetConfig(KEngineDefaultConfigs.AssetBundleExt);
                }
        ...
        //[MenuItem("Tools/Test")]
        public static void CopySceneToBundleResource()
        {
            string sourceScenePath = "Assets/Art/Scenes";
            string destPath = Application.dataPath + "/BundleResources/Scenes/";

            string[] lookFor = new string[] { sourceScenePath };
            string[] guids2 = AssetDatabase.FindAssets("t:scene", lookFor);

            foreach (string guid in guids2)
            {
                string scenePath = AssetDatabase.GUIDToAssetPath(guid);
                string sceneName = Path.GetFileName(scenePath);
                if (File.Exists(destPath + sceneName))
                {
                    File.Delete(destPath + sceneName);
                }
                File.Copy(scenePath, destPath + sceneName);
            }
        }
